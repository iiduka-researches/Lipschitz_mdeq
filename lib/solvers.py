...
def broyden(f, x0, threshold, eps=1e-6, stop_mode="rel", ls=False, name="unknown"):
    #print("broyden")
...
def anderson(f, x0, m=6, lam=1e-4, threshold=50, eps=1e-3, stop_mode='rel', beta=1.0, phase='train', **kwargs):
    ...
    final_nstep = lowest_step_dict[stop_mode] if trace_dict[stop_mode][-1] < eps else threshold
    wandb.log({f'anderson/{phase}_abs': abs_diff,
               f'anderson/{phase}_rel': rel_diff,
               f'anderson/{phase}_nstep': final_nstep})
