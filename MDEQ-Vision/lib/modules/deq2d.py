...
import copy
sys.path.append("../")
from lib.solvers import broyden, analyze_broyden, anderson
...
class DEQFunc2d(Function):
    ...
    def anderson_find_root(func, z1, u, eps, *args):
        ...
        f = lambda x: DEQFunc2d.list2vec(DEQFunc2d.f(func, DEQFunc2d.vec2list(x, cutoffs), u, *args))

        result_info = anderson(f, z1_est, threshold=threshold, eps=eps, stop_mode='rel', phase='train_forward' if train_step > -1 else 'test_forward', step=train_step)
        z1_est = result_info['result']
        nstep = result_info['nstep']
        lowest_step = result_info.get('lowest_step', nstep) 
        diff = result_info['lowest']

        if z1_est.get_device() == 0:
            if writer is not None:
                writer.add_scalar('forward/diff', diff, train_step)
                writer.add_scalar('forward/nstep', nstep, train_step)
    ...
    def forward(ctx, func, z1, u, *args):
        ...
        with torch.no_grad():
            ...
            z1_est = DEQFunc2d.anderson_find_root(func, z1, u, eps, *args)
