    ...
    if args.opts:
        ...
        for opt in args.opts:
            if '=' in opt:
                ...
                parsed_opts.extend([key, value])
            else:
                parsed_opts.append(opt)
        
        args.opts = parsed_opts
...
def main():
    ...
    wandb_project_name = "Lipschitz_mdeq_cifar"
    wandb_exp_name = f"Lipschitz MDEQ, ReLU={config.MODEL.RELU_SLOPE}"
    ...
    if dataset_name == 'imagenet':
        ...
        train_dataset = datasets.CIFAR10(root=f'{config.DATASET.ROOT}', train=True, download=False, transform=transform_train)
        valid_dataset = datasets.CIFAR10(root=f'{config.DATASET.ROOT}', train=False, download=False, transform=transform_valid)
